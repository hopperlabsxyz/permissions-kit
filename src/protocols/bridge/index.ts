...
function transfer( //We need: chainId, chainIdTarget, tokenAddress
  _: ChainId,
  targetAddress: TargetInfo,
) {
  return [
    //Step 1: approve USR resolv
// {
//     "func": "approve",
//     "params": [
...
export const eth = {
    transfer: async ({ targets }: { targets: TargetInfo[] }) => {
      return targets.flatMap((target) => transfer(1, getTargetInfo(target)));
    }
  }
